...
이제 해당 폴더(디렉토리 또는 공유)의 모든 파일에 액세스해야 하는 경우 백업 파일에서 폴더를 복원할 수 있습니다. 폴더 복원은 전체 볼륨을 복원하는 것보다 훨씬 효율적입니다. 이 기능은 Browse & Restore 메서드와 Search & Restore 메서드를 모두 사용하여 복원 작업에 사용할 수 있습니다. 현재 단일 폴더만 선택하고 복원할 수 있으며, 해당 폴더의 파일만 복원할 수 있습니다. 하위 폴더 또는 하위 폴더의 파일은 복원되지 않습니다.



==== 이제 아카이브 스토리지로 이동한 백업에서 파일 레벨 복구를 사용할 수 있습니다

과거에는 아카이브 저장소로 이동한 백업 파일(AWS 및 Azure만 해당)에서만 볼륨을 복원할 수 있었습니다. 이제 이러한 아카이빙된 백업 파일에서 개별 파일 및 폴더를 복원할 수 있습니다. 이 기능은 Browse & Restore 메서드와 Search & Restore 메서드를 모두 사용하여 복원 작업에 사용할 수 있습니다.
...
를 누릅니다 https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-viewing-azure-blob.html["StorageGRID"^] 백업 대상은 Canvas에 작업 환경으로 있으며, 온-프레미스 ONTAP 작업 환경을 대상으로 끌어서 백업 설정 마법사를 시작할 수 있습니다.
...
이메일로 알림을 수신하려면 동기화 관계에서 * 알림 * 설정을 활성화한 다음 Cloud Manager에서 알림 및 알림 설정을 구성해야 합니다.
...
Cloud Volumes ONTAP 시스템에서 AutoSupport 메시지를 보낼 수 없는 경우 Cloud Manager에서 알림을 생성합니다. 이 알림에는 네트워킹 문제를 해결하는 데 사용할 수 있는 지침 링크가 포함되어 있습니다.
