...
==== 랜섬웨어 보호 점수 패널에 새로운 권장 조치가 추가되었습니다

현재 귀사의 스토리지 시스템에서 발견되는 다음과 같은 랜섬웨어 보호 문제에 대해 권장되는 새로운 조치 두 개가 표시됩니다.

* _광범위한 권한이 있는 중요한 X 항목에 대한 사용 권한 감소_ - 열린 권한이 있는 민감한 파일이 데이터 원본에 있습니다
* _Patch X open CVE 전체 Y 데이터 소스 _ - 패치되지 않은 CVE가 ONTAP 시스템에서 발견되었습니다


UI에서 이러한 작업을 선택한 다음 워크플로를 따라 기본 문제를 해결할 수 있습니다. https://docs.netapp.com/us-en/cloud-manager-ransomware/task-analyze-ransomware-data.html#list-of-recommended-actions["모든 권장 조치 목록을 참조하십시오"].
...
랜섬웨어 보호 점수 패널은 잠재적인 문제가 있는 사이버 보안 영역의 전체 점수와 영역을 보여줍니다. _Recommended Actions_panel 은 랜섬웨어 공격에 대한 복원력을 높이기 위해 취할 수 있는 잠재적인 조치를 나열하고 필요한 경우 조치를 적용할 수 있도록 문제를 조사하는 링크를 제공합니다. 이 두 개의 새로운 패널은 데이터가 랜섬웨어 공격에 얼마나 잘 복원되는지, 그리고 점수를 개선하기 위해 무엇을 할 수 있는지 함께 확인합니다. https://docs.netapp.com/us-en/cloud-manager-ransomware/task-analyze-ransomware-data.html#ransomware-protection-score-and-recommended-actions["자세히 알아보기"^].
